...
function transformToLis(obj){
  let htmlArray = [];
  Object.entries(obj).forEach(property => htmlArray.push('<li>' + property.join(': ') + '</li>'));
  return htmlArray;
}
...
const addValues = (arr) => {
  let addedValues = arr.reduce((a, b, c) => {
    a = a + b;
    return a;
  }, 0)
  return addedValues;
...
const addPurchases = (arr) => {
  let purchasePrices = arr.reduce((a, b, c) => {
    a = a + b.purchasePrice;
    return a;
  }, 0)
...
const countNumberOfElements = (arr) => {
  let counter = arr.reduce(a => a + 1, 0)
  return counter;
};
...
const returnNames = (arr) => {
  let namesChar = arr.reduce((com, cur, i) => {
    com[i] = cur.name;
    return com
  }, [])
  return namesChar
