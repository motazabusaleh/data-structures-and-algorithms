...
const getNames = (arr) => {
  ...
  let theNewArray =  arr.map(value => value.name);
    let arrOfResult = [];
    theNewArray.forEach(ele=> arrOfResult.push(ele.split('').reverse().join('')));
    return arrOfResult
...
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
Write a function that, given an array of integer arrays as input, calculates the total sum of all the eles in the array.
...
This function should first remove any eles that are not numbers or are not divisible by five.
...
let findMaleAndFemale = (data) => {
  ...
  return data.filter(ele =>{if(ele.gender === 'male' || ele.gender === 'female') return ele;})
...
let findShortest = (data) => {
  ...
  return data.reduce((acc,ele) => {
    if(parseInt(ele.height) < acc['hight']) {
      acc['hight']=parseInt(ele.height);
      acc['e'] =ele.name;
    }
    return acc;
  },{hight:500,e:''}).e;
